...
func (d *diskHeap) init() error {
	file, err := os.OpenFile("db.dat", os.O_CREATE|os.O_RDWR, 0666)
	if err != nil {
		return err
	}
	d.file = file
...
func (d *diskHeap) mapSpace(offset int64, length int64, advice int, step ...uint) (space, error) {
	...
	// //grow
	// if d.fileSize.Load() < offset+length {
	// 	if err := d.grow(offset + length - d.fileSize.Load()); err != nil {
	// 		return space{}, err
	// 	}
	// }
...
func (d *diskHeap) mapUserSpace(offset int64, length int64, advice int) (space, error) {
	buf, err := syscall.Mmap(int(d.file.Fd()), ChunkSpaceOffset+offset, int(length), syscall.PROT_READ|syscall.PROT_WRITE, syscall.MAP_SHARED|syscall.MAP_ANONYMOUS)
	if err != nil {
		return space{}, err
	}
	...
	return space{buf: buf, step: 8}, nil
}
